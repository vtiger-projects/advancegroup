		...
		if(Users_Privileges_Model::isPermitted($moduleName, 'DetailView', $recordModel->getId())) {
			if ($moduleName == 'Quotes') {
				$detailViewLinks = array(
						'linklabel' => vtranslate('LBL_EXPORT_TO_PDF', $moduleName).' Tipo 1',
						'linkurl' => $recordModel->getExportPDFURL()."&Tipo=1",
						'linkicon' => ''
							);
				$linkModelList['DETAILVIEW'][] = Vtiger_Link_Model::getInstanceFromValues($detailViewLinks);
				
				$detailViewLinks = array(
						'linklabel' => vtranslate('LBL_EXPORT_TO_PDF', $moduleName).' Tipo 2',
						'linkurl' => $recordModel->getExportPDFURL()."&Tipo=2",
						'linkicon' => ''
							);
				$linkModelList['DETAILVIEW'][] = Vtiger_Link_Model::getInstanceFromValues($detailViewLinks);

				$sendEmailLink = array(
					'linklabel' => vtranslate('LBL_SEND_MAIL_PDF', $moduleName).' Tipo 1',
					'linkurl' => 'javascript:Inventory_Detail_Js.sendEmailPDFClickHandler(\''.$recordModel->getSendEmailPDFUrl().'&Tipo=1'.'\')',
					'linkicon' => ''
				);
				$linkModelList['DETAILVIEW'][] = Vtiger_Link_Model::getInstanceFromValues($sendEmailLink);
				
				$sendEmailLink = array(
					'linklabel' => vtranslate('LBL_SEND_MAIL_PDF', $moduleName).' Tipo 2',
					'linkurl' => 'javascript:Inventory_Detail_Js.sendEmailPDFClickHandler(\''.$recordModel->getSendEmailPDFUrl().'&Tipo=2'.'\')',
					'linkicon' => ''
				);
				$linkModelList['DETAILVIEW'][] = Vtiger_Link_Model::getInstanceFromValues($sendEmailLink);
			} else {
